...
UBTTask_RangedAttack::UBTTask_RangedAttack(const FObjectInitializer& object_intialize)
{
	NodeName = TEXT("UBTTask_RangedAttack");
}

EBTNodeResult::Type UBTTask_RangedAttack::ExecuteTask(UBehaviorTreeComponent& owner_comp, uint8* node_memory)
{
	...
	if (SelectedAnim && Boss)
	{
		...
		TimerDel.BindUObject(this, &UBTTask_RangedAttack::OnTimerFinished, &owner_comp);
...
void UBTTask_RangedAttack::OnTimerFinished(UBehaviorTreeComponent* owner_comp)
{
